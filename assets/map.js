...
$(document).ready(() => {
  ...
  // global object with annotations for gonfaloni so it can be accessed later
  const regionObjects = [];
  // how to log objects so you can see the content:
  // console.log('regionObjects: ' + JSON.stringify(regionObjects)) 

  // get data for annotations from JSON here so it runs just once, not every time a checkbox is checked. The code to access the JSON file and reformat the pointsString was partly written by ChatGPT.
      ...
      if ($(this).prop("checked") == true) {
        ...
        regionObjects.forEach(obj => {
          ...
          if (obj.type === "Quartieri") {
            ...
            var styleString = "fill:none;stroke:black;stroke-width:4"
          }

          if (obj.type === "Gonfaloni") {
            // Different line style for Gonfaloni: stroke-dasharray means dashed line
          ...
          $('#oldmap-container').append($('<div class="svg-container" id="generated"><svg viewBox="0 0 2178 2121" preserveAspectRatio="none"><polygon points=" ' + pointsString + ' "style=' + styleString + ' /><text x="200" y="100" fill="black">' + nameString + '</text></svg></div>'))
        });
